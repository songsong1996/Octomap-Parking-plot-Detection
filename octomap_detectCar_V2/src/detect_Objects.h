...
//            findContours(im,contours_list,hierarchy,RETR_EXTERNAL,CHAIN_APPROX_SIMPLE);
//            bool has_wall=false;
//            for(int i=0;i<contours_list.size();i++)
//                if( boundingRect(Mat(contours_list[i])).width > 20  ||  boundingRect(Mat(contours_list[i])).height > 20)
//                    has_wall=true;
        ...
        while(iti!=image_list_orig.end())
        {
            ...
            //                        tp.at<uchar>(i,j)=255;
            //            cv::namedWindow("o",CV_WINDOW_NORMAL);
            //            cv::imshow("o",tp);
            //            cv::waitKey(0);
            //            cv::erode(tp,tp,Mat());
...
//            cv::waitKey(0);
    ...
    void detect_wall_Pillar_car()
    {
        //detect the wall
        // cv::Mat contours;
        // cv::Canny(image_list_after_rotate[1],contours,700,800);
//        imshow("tp",contours);
//        waitKey(0);
        vector<Vec4i> lines;
...
//        waitKey(0);
        ...
        while(itm!=image_list_after_rotate.end())
        {
            ...
            //contours=contours_tp;
            //merge_cntours(contours_tp,contours);
////            findContours(image_list_after_rotate[0],contours,hierarchy,RETR_EXTERNAL,CHAIN_APPROX_SIMPLE);
//            Mat dstImage;
//            cv::cvtColor(image_list_after_rotate[0],dstImage,COLOR_GRAY2BGR);
//            for (int i = 0; i < contours.size(); i++)
//            { Scalar color = Scalar(rand() % 255, rand() % 255, rand() % 255);
//                for(int j=0;j<contours[i].size()-1;j++)
//                    cv::line(dstImage, contours[i][j],contours[i][j+1], color); }
//                    cv::namedWindow("tp",WINDOW_NORMAL);
//            imshow("tp", dstImage);
//            waitKey(0);
...
//                    Scalar color = Scalar(rand() % 255, rand() % 255, rand() % 255);
//                    cv::rectangle(dstImage, boundingRect(Mat(contours[i])),color);
                }
            }
//            cv::namedWindow("tp",WINDOW_NORMAL);
//            imshow("tp", dstImage);
//            waitKey(0);
        ...
        for(int j=0;j<mcontour_list[1].size();j++)
        {
            if(  mcontour_list[1][j].type!=1 && mcontour_list[1][j].type!=2)//car
            {
                ...
                if(contourArea(mcontour_list[1][j].point_list)>(1.0/resolution)*(1.0/resolution))
                {
...
//        Mat dstImage;
//        cv::cvtColor(image_list_after_rotate[0],dstImage,COLOR_GRAY2BGR);
//        for(int i=0;i<mcontour_list[0].size();i++)
//            if(mcontour_list[0][i].type==2)
//                cv::rectangle(dstImage,boundingRect( Mat(mcontour_list[1][i].point_list) ),cv::Scalar(255,255,0));
//        imshow("tp",dstImage);
//        waitKey(0);
//        imshow("tp1",image_list_after_rotate[0]);
//        waitKey(0);

//        Mat dstImage;
//        cv::cvtColor(image_list_after_rotate[1],dstImage,COLOR_GRAY2BGR);
//        for(int i=0;i<mcontour_list[1].size();i++)
//            if(mcontour_list[1][i].type==3)
//                cv::rectangle(dstImage,boundingRect( Mat(mcontour_list[1][i].point_list) ),cv::Scalar(255,255,0));
//        imshow("tp",dstImage);
//        waitKey(0);
        ...
        //Mat dstImage;
//        cv::cvtColor(image_list_after_rotate[1],dstImage,COLOR_GRAY2BGR);
//        for(Rect r: pillar_boundingRect )
//        { cv::rectangle( dstImage, r, Scalar(0,0,255), 1, CV_AA); }
//        imshow("tp",dstImage);
//        waitKey(0);
...
//                            if(pillar_border.width<2.0/resolution && pillar_border.height<2.0/resolution && calRectAngle(pillar_border)>5.0/180.0*CV_PI)
//                                (*itc1).type = 2;
//                            else
//                            {
//                                (*itc1).type = 1;
...
//                            if (pillar_border.x + pillar_border.width / 2.0 > r2.x &&
//                                pillar_border.x + pillar_border.width / 2.0 < r2.x + r2.width
//                                && pillar_border.y + pillar_border.height / 2.0 > r2.y &&
//                                pillar_border.y + pillar_border.height / 2.0 < r2.y + r2.height) {
////                                if(cv::contourArea((*itc2).point_list)/cv::contourArea((*itc1).point_list)<1.5)
//                                if(double(r2.width*r2.height)/double(pillar_border.width*pillar_border.height)<2.0 || (r2.width<0.6/resolution && r2.height<0.6/resolution) )
//                                {
//                                    (*itc2).type = 2;
//                                    (*itc1).type_id = pillar_id;
//                                    (*itc2).type_id = pillar_id;
//                                    pillar_id++;
//                                }
//                            }
                        }
...
//        cout<<rotate_mat<<endl;
        ...
        //cout<<offset_mat<<endl;
//        cv::Rect bbox = cv::RotatedRect(center, image_list_after_rotate[0].size(), -rotate_angle/CV_PI*180.0).boundingRect();
    ...
    void compute_car_space()
    {
        ...
        if(wall_boundingRect.size()>0) {
            for (Rect2d wall:wall_boundingRect)
                if (std::abs(calRectAngle(wall)) > 80.0 / 180.0 * CV_PI)//verticle
                    wall_verticle = wall;
            if(car_boundingRect.size()>0)
            {
                double car_wall_min_dist = std::abs(wall_verticle.x - car_boundingRect[0].x);
                ...
                if (car_wall_min_dist > 2.5 / resolution)
        ...
        for(Rect2d r1:obstacles_list)
        {
            ...
            for(Rect2d r2:obstacles_list)
                ...
                {
                    if(dist_max<std::abs(r1.x+r1.height/2.0-r2.x-r2.height/2.0))
                    {
                        dist_max=std::abs(r1.x+r1.height/2.0-r2.x-r2.height/2.0);
                    }
                }
        }
        ...
        {
            if(calRectAngle(wall_boundingRect[i])<CV_PI/3.0)
                obstacles_list.emplace_back(wall_boundingRect[i]);
        }

        if(dist_max>car_norm.width_pass/resolution) //can divide
        {
...
//        Mat dstImage;
//        cv::cvtColor(image_list_after_rotate[1],dstImage,COLOR_GRAY2BGR);
//        for(Rect2d r:obstacles_list_left)
//        {
//            cv::rectangle(dstImage,r,cv::Scalar(255,255,0));
//        }
//        for(Rect2d r:obstacles_list_right)
//        {
//            cv::rectangle(dstImage,r,cv::Scalar(255,0,255));
//        }
////        for(Rect2d r:obstacles_list)
////        {
////            cv::rectangle(dstImage,r,cv::Scalar(0,0,255));
////        }
//        imshow("tp",dstImage);
//        waitKey(0);
...
//            waitKey(0);
            ...
            {
                if (it1 != it2 && (*it2).y+(*it2).height>(*it1).y+(*it1).height)
                {
                    //double dist_tp=calRectDist(*it1,*it2);
                    if (calRectDist(*it1, *it2) < obsticles_min_dist) {
                        change_dist=true;
                        obsticles_min_dist = calDist(*it1, *it2);
                        obs2 = *it2;
                        id2 = i2;
                    }
                }
...
//            waitKey(0);
            ...
            if (wall_boundingRect.size() > 0) {//judge if near the wall
                if(change_dist) {
...
//                {
                if (near_the_wall) {
                    if (wall.tl().x - tp_p1.x > 0)
                        ...
                        x = wall.br().x;
//                                car_tp.tl() = Point2d(x, car_boundingRect[min_id_c].br().y +
...
//                                car_tp.tl() = Point2d(x, car_boundingRect[min_id_p].br().y +
//                                                         i * car_norm.length / resolution);
                    car_tp.width = car_norm.width_near_the_space / resolution;
                    car_tp.y = r2.br().y + i * car_norm.length / resolution;
                    car_tp.x = x;
...
//        {if(r1.tl().y-r2.br().y>0)return r1.tl().y-r2.br().y;
//            else return 0;}
//        else
//        {if(r2.tl().y-r1.br().y>0)return r2.tl().y-r1.br().y;
//            else return 0;}
    }
    //计算两点间的距离
...
//    void Rect2List(Rect & rect,vector<Point2d> &output)
//    {
//        output.empty();
//        output.emplace_back(rect.tl());
//        output.emplace_back(Point2d(rect.tl().x+rect.width,rect.tl().y));
//        output.emplace_back(rect.br());
//        output.emplace_back(Point2d(rect.tl().x,rect.tl().y+rect.height));
//    }
    ...
    void merge_rect(vector<Rect2d>& rect_input,vector<Rect2d>& rect_output,double thres=4.0)
    {
...
//        waitKey(0);
        ...
        for(int i=0;i<rect_output.size();i++)
        {
            if(calRectAngle(rect_output[i])>60.0/180.0*CV_PI)//VERTICLE
            {
                ...
                {
                    int left_x=int(rect_output[i].x)-8>0?int(rect_output[i].x)-8:0;
                    int right_x=int(rect_output[i].x+3+8)<image_list_after_rotate[j].cols?
                                int(rect_output[i].x+3+8):image_list_after_rotate[j].cols;
    ...
    void draw(int typet,string name="")
    {
        ...
        } else if(typet==2)
        {
            for (int i = 0; i < image_list_after_rotate.size(); i++)
            {
                ...
                if(name!="")
                    ...
                    cv::namedWindow("show", WINDOW_NORMAL);
                    cv::imshow("show", tp);
                   // cv::imwrite("../data/image/find_objects" + std::to_string(i + 1) + ".jpg", tp);
                    cv::waitKey(0);
                }
            }
        }
        else if (typet==3)
        {
            for (int i = 0; i < image_list_orig.size(); i++)
            {
                ...
                cv::namedWindow("show", WINDOW_NORMAL);
                cv::imshow("show", tp);
                cv::imwrite("../find_objects.jpg", tp);
                cv::waitKey(0);
            }
        }
    }
